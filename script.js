...
(function () {
  ...
  function initNav() {
    ...
    /* Phone: closing the menu when the user scrolls the page */
  ...
  /* ---------- Mobile: transparent header + white text only over #hero ---------- */
  function initMobileHeaderHero() {
    ...
    var io = new IntersectionObserver(
      function (entries) {
        entries.forEach(function (entry) {
          ...
          if (entry.isIntersecting) header.classList.add("header-over-hero");
          else header.classList.remove("header-over-hero");
        });
      },
    ...
    window.addEventListener("resize", function () {
      if (!mq.matches) header.classList.remove("header-over-hero");
    });
  }
  ...
  function initSmoothScroll() {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(function (anchor) {
      anchor.addEventListener("click", function (e) {
        ...
        e.preventDefault();
        /* Mobile header scrolls with the page (not sticky); only small top padding. */
        var pad =
          window.innerWidth <= 768
            ? 12
            : (header ? header.offsetHeight : 0) + 8;
        var top = target.getBoundingClientRect().top + window.scrollY - pad;
